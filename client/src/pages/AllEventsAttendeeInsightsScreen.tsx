...
import { SearchIcon, FilterIcon, UsersIcon, CalendarIcon, MapPinIcon, DollarSignIcon } from "lucide-react";
...
interface AllEventsInsights {
	totalAttendees: number;
	totalBookings: number;
	totalEvents: number;
	totalRevenue: number;
	averageTicketsPerBooking: number;
	ageDistribution: Array<{ ageRange: string; count: number; percentage: number; color: string }>;
	...
	topEvents: Array<{ event: Event; bookings: number; revenue: number }>;
	monthlyTrends: Array<{ month: string; bookings: number; revenue: number }>;
}

export const AllEventsAttendeeInsightsScreen = (): JSX.Element => {
	...
	// Fetch all bookings across all events
	...
	const calculateInsights = (): AllEventsInsights => {
		...
		const ageDistribution = Object.entries(ageGroups).map(([range, count]) => ({
			...
			color: range === "18-24" ? "#6366f1" : range === "25-34" ? "#8b5cf6" : range === "35-44" ? "#06b6d4" : "#10b981"
		}));
		...
		const eventStats: Record<string, { event: Event; bookings: number; revenue: number }> = {};
		bookings.forEach(booking => {
			...
			eventStats[eventId].bookings++;
			eventStats[eventId].revenue += booking.totalAmount;
		});
		...
		bookings.forEach(booking => {
			const month = new Date(booking.bookingDate).toLocaleDateString('en-US', { month: 'short', year: 'numeric' });
	...
	// Filter bookings based on search, status, and event
	const filteredBookings = bookings.filter(booking => {
		...
		const matchesEvent = filterEvent === "all" || booking.event._id === filterEvent;
					...
					<main className="flex-1 p-6 bg-[#f2f2f2]">
						<div className="bg-white rounded-[15px] p-8 min-h-[calc(100vh-140px)]">
							<div className="text-center py-8">
								...
								<p className="mt-4 text-gray-600">Loading all events insights...</p>
							</div>
						</div>
					</main>
				</div>
					...
					<main className="flex-1 p-6 bg-[#f2f2f2]">
						<div className="bg-white rounded-[15px] p-8 min-h-[calc(100vh-140px)]">
							<div className="text-center py-8">
								...
								<p className="text-gray-600">There was an error loading the all events insights.</p>
								<Button 
		...
		<div className="bg-[#f0f0f0] flex w-screen min-h-screen">
			<div className="bg-[#f0f0f0] w-full min-h-screen flex">
				...
				<div className="flex-1 flex flex-col">
					...
					{/* All Events Attendee Insights Content */}
					<main className="flex-1 p-6 bg-[#f2f2f2]">
						<div className="bg-white rounded-[15px] p-8 min-h-[calc(100vh-140px)]">
							{/* Page Header */}
							<div className="flex items-center justify-between mb-8">
								<div className="flex items-center gap-4">
									<h1 className="[font-family:'Poppins',Helvetica] font-bold text-black text-3xl tracking-[0] leading-[normal]" data-testid="heading-all-attendee-insights">
										All Events Attendee Insights
									</h1>
								</div>
								<div className="flex items-center gap-3">
									<Badge variant="secondary" className="px-4 py-2 text-sm" data-testid="badge-total-attendees">
										<UsersIcon className="w-4 h-4 mr-2" />
										Total Attendees: {insights.totalAttendees.toLocaleString()}
									</Badge>
							...
							<div className="flex items-center gap-3 mb-8">
								<div className="relative">
									<Input
										...
										className="pl-10 pr-4 py-2 w-80 rounded-[8px] border border-gray-300"
										data-testid="input-search-all-insights"
									/>
							...
							<div className="grid grid-cols-4 gap-6 mb-8">
								<div className="bg-blue-50 p-6 rounded-[8px] border border-blue-200">
									<div className="flex items-center gap-3">
										<UsersIcon className="w-8 h-8 text-blue-600" />
										<div>
											<p className="text-sm text-blue-600 font-medium">Total Events</p>
											<p className="text-2xl font-bold text-blue-800">{insights.totalEvents}</p>
										</div>
									</div>
								</div>
								<div className="bg-green-50 p-6 rounded-[8px] border border-green-200">
									<div className="flex items-center gap-3">
										<CalendarIcon className="w-8 h-8 text-green-600" />
										<div>
											<p className="text-sm text-green-600 font-medium">Total Bookings</p>
											<p className="text-2xl font-bold text-green-800">{insights.totalBookings}</p>
										</div>
									</div>
								</div>
								<div className="bg-purple-50 p-6 rounded-[8px] border border-purple-200">
									<div className="flex items-center gap-3">
										<DollarSignIcon className="w-8 h-8 text-purple-600" />
										<div>
											<p className="text-sm text-purple-600 font-medium">Avg Tickets/Booking</p>
											<p className="text-2xl font-bold text-purple-800">{insights.averageTicketsPerBooking}</p>
										</div>
									</div>
								</div>
								<div className="bg-orange-50 p-6 rounded-[8px] border border-orange-200">
									<div className="flex items-center gap-3">
										<MapPinIcon className="w-8 h-8 text-orange-600" />
										<div>
											<p className="text-sm text-orange-600 font-medium">Active Events</p>
											<p className="text-2xl font-bold text-orange-800">{events.filter(e => new Date(e.date) > new Date()).length}</p>
										</div>
									</div>
								</div>
							</div>
									...
									{insights.topEvents.length > 0 && (
										<div>
											...
											<div className="space-y-3">
												{insights.topEvents.map((eventStat, index) => (
													<div key={eventStat.event._id} className="flex items-center justify-between p-3 bg-gray-50 rounded-[8px]">
														<div className="flex items-center gap-3">
															<Badge variant="outline" className="w-6 h-6 p-0 flex items-center justify-center text-xs">
																{index + 1}
															</Badge>
															<div>
																<p className="text-sm font-medium">{eventStat.event.title}</p>
																<p className="text-xs text-gray-500">{formatDate(eventStat.event.date)}</p>
															</div>
														</div>
														<div className="text-right">
															<span className="text-lg font-bold">{eventStat.bookings}</span>
										...
										<div>
											...
											<div className="h-64 w-full flex items-center justify-center bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
												<div className="text-center text-gray-500">
													...
													<p className="text-xs">Payment information is not available</p>
												</div>
											</div>
										</div>
									)}
								...
								<div className="mb-8">
									...
									<div className="h-64 w-full flex items-center justify-center bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
										<div className="text-center text-gray-500">
											...
											<p className="text-xs">Monthly trends will appear here when you have more bookings</p>
										</div>
									</div>
								</div>
							)}
							...
							{filteredBookings.length > 0 && (
								<div>
									...
									<div className="bg-gray-50 rounded-[8px] p-4">
										<div className="grid grid-cols-7 gap-4 mb-4 font-medium text-sm text-gray-600">
											<div>Event</div>
											<div>Attendee</div>
											<div>Email</div>
											<div>Age</div>
										...
										{filteredBookings.map((booking) => (
											<div key={booking._id} className="grid grid-cols-7 gap-4 py-3 border-b border-gray-200 last:border-b-0">
												<div className="font-medium">
													{booking.event.title}
													<div className="text-xs text-gray-500">{formatDate(booking.event.date)}</div>
												</div>
												<div className="font-medium">
							...
							{insights.totalBookings === 0 && (
								<div className="text-center py-12">
									...
									<p className="text-gray-500">Your events don't have any bookings yet.</p>
								</div>
							)}
						</div>
					</main>
